...
class brain_python:
    def __init__(self, n_times, n_frequency, n_windows, n_outputs, n_neurons, learning_rate):
        ...
        self.X_STFT = tf.reshape(self.X,[-1,self.line_of_frequency,self.window,1])
        
        self.Y = tf.placeholder(tf.float32,[None, self.outputs])

        self.keep_prob = tf.placeholder_with_default(1.0, shape=())
        
        self.W1 = tf.Variable(tf.random_normal([3,3,1,64]))
        self.L1 = tf.nn.conv2d(self.X_STFT,self.W1,strides=[1,1,1,1],padding='VALID')
        self.L1 = tf.nn.relu(self.L1)
        self.L1 = tf.nn.max_pool(self.L1,ksize=[1,2,1,1],strides=[1,2,1,1],padding='VALID') 
        self.L1 = tf.reduce_max(self.L1,reduction_indices=[3],keep_dims=True)
        print(self.L1.shape)
        self.CNN_output = tf.transpose(self.L1,[0,3,1,2])
        self.CNN_output = tf.reshape(self.CNN_output,[-1,36,28])
        ...
        self.cells_drop = [rnn_cell.DropoutWrapper(cell, output_keep_prob = self.keep_prob) for cell in self.cells]
        self.multi_layer_cell = rnn_cell.MultiRNNCell(self.cells_drop)
        self.lstm_outputs, states = tf.nn.dynamic_rnn(self.multi_layer_cell, self.CNN_output, dtype = tf.float32)
        
        self.dense_outputs = tf.layers.dense(self.lstm_outputs[:,-1,:], self.outputs)
        self.softmax_outputs = tf.nn.softmax(self.dense_outputs)
...
#         return accuracy, correct_list, fail_list, unknown_list
    ...
    def PCA_1d(self,data):
        scaler = StandardScaler()
        pca_1 = PCA(n_components=1)

        pca_data = np.zeros([data.shape[0],data.shape[1],1])
        for i in range(data.shape[0]):
            pca_data[i] = pca_1.fit_transform(data[i])
        return pca_data
