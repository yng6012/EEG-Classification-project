...
#         self.X = tf.placeholder(tf.float32,[None, 100, self.window])
        self.Y = tf.placeholder(tf.float32,[None, self.outputs])

        self.keep_prob = tf.placeholder_with_default(1.0, shape=())

        #for loop의 layer를 [128,64,32]와 같이 바꿔 봄으로 서, num_units 만져보기.
        self.cells = [rnn_cell.LSTMCell(num_units = neurons, activation=tf.nn.softsign) for neurons in n_neurons]
